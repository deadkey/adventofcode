...
def get_args(argv):
    ...
    for arg in argv[1:]:
        if arg == 'manual':
           cmds.append('manual') #force fetch
        if arg == 'f':
           cmds.append(FF) #force fetch
        if arg == 'p' or arg == 'pi':
           cmds.append(PR) #print input
        if arg == 'so':
            so = 1 #samples only
        if arg == 'io':
            io = 1 #input only
...
def run_samples(p1_fn, p2_fn, cmds):
    default = 'p1' not in cmds and 'p2' not in cmds or 'manual' in cmds
...
def run(YEAR, DAY, p1_fn, p2_fn, cmds = {}):
    ...
    res = res1 if part1 else res2
    pyperclip.copy(res)

    if 'submit' in cmds and part1:
        answer(YEAR, DAY, 1, res1)
    if 'submit' in cmds and part2:
